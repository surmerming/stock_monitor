...
export default function BacktestPage() {
  ...
  const [filters, setFilters] = useState<ScreenerFilter[]>(PRESET_STRATEGIES[0].filters);
  ...
  const [activePreset, setActivePreset] = useState(PRESET_STRATEGIES[0].name);
  ...
  }, [symbol, filters, startDate, endDate, initialCapital, positionSize, stopLoss, takeProfit, holdDays]);
        ...
        <div className="bt-page__main">
          ...
          <div className="bt-filters">
            <div className="bt-filters__head">
              ...
              <button className="bt-filters__add" onClick={addFilter}>+ 添加指标</button>
            </div>
            {filters.map((f, i) => (
              <div key={i} className="bt-filter-row">
                ...
                <select
                  ...
                  value={f.operator}
                  onChange={(e) => updateFilter(i, { operator: e.target.value as ScreenerFilter['operator'] })}
                >
                  {OPERATORS.map((op) => (
                ...
                <button className="bt-filter-row__del" onClick={() => removeFilter(i)}>×</button>
              </div>
            ))}
          </div>
          ...
          {result && (
            <div className="bt-result">
              ...
              <div className="bt-stats">
                <StatCard label="总收益" value={`${result.stats.totalReturn >= 0 ? '+' : ''}${result.stats.totalReturn.toFixed(2)}%`} trend={result.stats.totalReturn >= 0 ? 'up' : 'down'} />
                <StatCard label="基准收益" value={`${result.stats.benchmarkReturn >= 0 ? '+' : ''}${result.stats.benchmarkReturn.toFixed(2)}%`} trend={result.stats.benchmarkReturn >= 0 ? 'up' : 'down'} />
                <StatCard label="年化收益" value={`${result.stats.annualizedReturn >= 0 ? '+' : ''}${result.stats.annualizedReturn.toFixed(2)}%`} trend={result.stats.annualizedReturn >= 0 ? 'up' : 'down'} />
                <StatCard label="最大回撤" value={`-${result.stats.maxDrawdown.toFixed(2)}%`} trend="down" />
                <StatCard label="夏普比率" value={result.stats.sharpeRatio.toFixed(2)} />
                <StatCard label="胜率" value={`${result.stats.winRate.toFixed(1)}%`} trend={result.stats.winRate >= 50 ? 'up' : 'down'} />
                <StatCard label="盈亏比" value={result.stats.profitFactor === Infinity ? '∞' : result.stats.profitFactor.toFixed(2)} />
                <StatCard label="交易次数" value={`${result.stats.totalTrades}`} />
                <StatCard label="平均持仓" value={`${result.stats.avgHoldDays.toFixed(1)}天`} />
                <StatCard label="平均盈亏" value={`${result.stats.avgPnlPercent >= 0 ? '+' : ''}${result.stats.avgPnlPercent.toFixed(2)}%`} trend={result.stats.avgPnlPercent >= 0 ? 'up' : 'down'} />
                <StatCard label="最大单笔盈利" value={`+${result.stats.maxWin.toFixed(2)}%`} trend="up" />
                <StatCard label="最大单笔亏损" value={`${result.stats.maxLoss.toFixed(2)}%`} trend="down" />
              </div>
...
function TradeRow({ trade, index }: { trade: BacktestTrade; index: number }) {
  ...
  return (
    <div className={`bt-trades__row bt-trades__row--${trend}`}>
      ...
      <span className={`bt-trades__col bt-trades__col--pnl bt-trades__${trend}`}>
        {isWin ? '+' : ''}{formatTurnover(trade.pnl)}
      </span>
      <span className="bt-trades__col bt-trades__col--hold">{trade.holdDays}</span>
      <span className="bt-trades__col bt-trades__col--reason">{EXIT_LABELS[trade.exitReason] || trade.exitReason}</span>
    </div>
  );
}
