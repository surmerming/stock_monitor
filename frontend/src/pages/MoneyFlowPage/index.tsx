...
import {
  MONEYFLOW_MARKET_TABS,
  MONEYFLOW_SORT_OPTIONS,
  MONEYFLOW_REFRESH_INTERVAL,
  type MoneyFlowSortKey,
  type MoneyFlowMarketFilter,
} from '../../configs/moneyflow';
import './style.less';
...
export default function MoneyFlowPage() {
  const [market, setMarket] = useState<MoneyFlowMarketFilter>('all');
  const [sortKey, setSortKey] = useState<MoneyFlowSortKey>('netFlow');
  ...
  const fetchOverview = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/moneyflow/overview${qs ? `?${qs}` : ''}`);
  ...
  useEffect(() => {
    ...
    fetchOverview();
    if (isCurrentDay) {
      const timer = setInterval(fetchOverview, MONEYFLOW_REFRESH_INTERVAL);
  ...
  const fetchDetail = useCallback(
    async (symbol: string) => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/moneyflow/${encodeURIComponent(symbol)}${params}`);
      ...
      <div className="mf-page__filters">
        <div className="mf-page__filters-left">
          ...
          <div className="mf-page__date-picker">
            ...
            />
            {!isCurrentDay && (
              <button className="mf-page__date-today" onClick={() => setSelectedDate(todayStr())}>
                回到今天
              </button>
            )}
          </div>
        ...
        <div className="mf-page__sorts">
          ...
          {MONEYFLOW_SORT_OPTIONS.map((opt) => (
            <button
              key={opt.key}
        ...
        {detailSymbol && (
          <div className="mf-page__detail">
            <div className="mf-page__detail-header">
              <h3 className="mf-page__detail-title">{detail?.name || detailSymbol} 资金明细</h3>
              <button className="mf-page__detail-close" onClick={() => setDetailSymbol(null)}>
                ✕
              </button>
            </div>
            {detailLoading ? (
        ...
        return (
          ...
          >
            ...
            <span className="mf-table__col mf-table__col--market">
              <span
                className={`mf-table__market-tag mf-table__market-tag--${marketCssKey(item.market)}`}
              >
                {item.market || '—'}
              </span>
            </span>
            ...
            <span className="mf-table__col mf-table__col--price">{item.price.toFixed(2)}</span>
    ...
    <div className="mf-detail">
      <div className="mf-detail__summary">
        <div className="mf-detail__stat mf-detail__stat--main">
          ...
          <span
            className={`mf-detail__stat-value ${netPositive ? 'mf-detail__inflow' : 'mf-detail__outflow'}`}
          >
            {netPositive ? '+' : ''}
            {formatTurnover(summary.netFlow)}
          </span>
        </div>
        <div className="mf-detail__stat">
          <span className="mf-detail__stat-label">大额净流入</span>
          <span
            className={`mf-detail__stat-value ${largeNetPositive ? 'mf-detail__inflow' : 'mf-detail__outflow'}`}
          >
            {largeNetPositive ? '+' : ''}
            {formatTurnover(summary.largeNetFlow)}
          </span>
        </div>
        <div className="mf-detail__stat">
        ...
        <div className="mf-detail__large-orders">
          ...
          <div className="mf-detail__orders-list">
            {largeBars.slice(0, 20).map((bar, i) => (
              <div key={i} className={`mf-detail__order mf-detail__order--${bar.direction}`}>
                <span className="mf-detail__order-time">
                  {new Date(bar.time).toLocaleTimeString('zh-CN', { hour12: false })}
                </span>
                <span
                  className={`mf-detail__order-dir ${bar.direction === 'buy' ? 'mf-detail__inflow' : 'mf-detail__outflow'}`}
                >
