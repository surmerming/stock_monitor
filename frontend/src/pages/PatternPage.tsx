...
const DIRECTION_LABELS: Record<string, { label: string; cls: string }> = {
  bullish: { label: '看涨', cls: 'up' },
  bearish: { label: '看跌', cls: 'down' },
...
export default function PatternPage() {
  ...
  const detect = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/pattern/${encodeURIComponent(symbol.trim())}?range=${range}`);
    ...
    <div className="pt-page">
              ...
              <div className="pt-trends">
                {result.trendLines.map((tl, i) => (
                  <div key={i} className={`pt-trend pt-trend--${tl.type}`}>
                    ...
                    <span className="pt-trend__type">{tl.type === 'up' ? '上升趋势线' : '下降趋势线'}</span>
                    <span className="pt-trend__range">
                      {tl.startDate} ({tl.startPrice.toFixed(2)}) → {tl.endDate} ({tl.endPrice.toFixed(2)})
                    </span>
                  </div>
                ))}
              </div>
      ...
      {!result && !loading && !error && (
        <div className="pt-placeholder">
          ...
          <div className="pt-placeholder__text">输入股票代码，自动识别K线形态、支撑阻力位和趋势线</div>
  ...
  return (
    <div className={`pt-sr ${isSupport ? 'pt-sr--support' : 'pt-sr--resist'}`}>
      ...
      <div className="pt-sr__bar">
        <div className={`pt-sr__bar-fill ${isSupport ? 'pt-sr__bar-fill--support' : 'pt-sr__bar-fill--resist'}`}
