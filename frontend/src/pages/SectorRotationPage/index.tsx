...
import { SECTOR_MARKETS, PERIOD_TABS } from '../../configs/sector';
import './style.less';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [rotRes, heatRes] = await Promise.all([
        apiFetch(`/api/sector/rotation?market=${encodeURIComponent(market)}`),
        apiFetch(`/api/sector/heatmap?market=${encodeURIComponent(market)}`),
      ]);
  ...
  return (
    <div className="sr-page">
      ...
      <div className="sr-page__tabs">
        <button
          className={`sr-tab ${tab === 'rotation' ? 'sr-tab--active' : ''}`}
          onClick={() => setTab('rotation')}
        >
          轮动排行
        </button>
        <button
          className={`sr-tab ${tab === 'heatmap' ? 'sr-tab--active' : ''}`}
      ...
      {tab === 'rotation' && (
        <>
          ...
          {loading && rotation.length === 0 && <div className="sr-loading">加载中...</div>}
            ...
            {sortedRotation.map((item, i) => (
              <div key={item.symbol} className="sr-table__row">
                ...
                <span className="sr-table__col sr-table__col--score">
                  <span
                    className={`sr-score ${item.rsScore >= 0 ? 'sr-score--up' : 'sr-score--down'}`}
                  >
